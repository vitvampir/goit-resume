...
function calculateSum(){
        ...
        const websiteReception = document.querySelector('.calculator-form-reception input:checked')
    
    
    
        //Values
        console.log(extractPriceFromValue(websiteTypeSelect.nodeValue));
        const websiteTypeValue = extractPriceFromValue(websiteTypeSelect.nodeValue);
        ...
        const wesiteCarReception = convertRecepcionOptionToPric(websiteReception.value);
    
        const totalSum = websiteTypeValue + technologiesValue + wesiteCartValue + wesiteCarReception;
...
function renderSum(sum){
    const costElement = document.querySelector('.calculator-form-totel-cost');
...
function convertRecepcionOptionToPrice(options){
    if (options ==='yes'){
        return 500;
    }
